...
export function serializeSharedSecretFrame(
      frame: SharedSecretFrame
      ): ByteBuffer {
...
export function deserializeSharedSecretFrame(
	buffer: ByteBuffer,
	flags: number,
	...
	const sharedSecret = readBytes(buffer, offset, SHARED_SECRET_SIZE);
	offset += SHARED_SECRET_SIZE;

	const token = readBytes(buffer, offset, TOKEN_SIZE);
	
	return {
		type: FRAME_TYPES.SHARED_SECRET,
		flags,
		publicKey,
		sharedSecret,
		seqId,
