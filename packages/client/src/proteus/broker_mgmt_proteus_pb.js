...
var BrokerManagementServiceClient = function () {
  ...
  BrokerManagementServiceClient.prototype.shutdown = function shutdown(message, metadata) {
    ...
    var metadataBuf = proteus_js_frames.encodeProteusMetadata('io.netifi.proteus.broker.info.BrokerManagementService', 'shutdown', Buffer.alloc(0), metadata || Buffer.alloc(0));
  ...
  BrokerManagementServiceClient.prototype.leave = function leave(message, metadata) {
    var dataBuf = Buffer.from(message.serializeBinary());
    var metadataBuf = proteus_js_frames.encodeProteusMetadata('io.netifi.proteus.broker.info.BrokerManagementService', 'leave', Buffer.alloc(0), metadata || Buffer.alloc(0));
    const map = {};
    return this.leaveTrace(map)(
    ...
    BrokerManagementServiceClient.prototype.rejoin = function rejoin(message, metadata) {
      var dataBuf = Buffer.from(message.serializeBinary());
      var metadataBuf = proteus_js_frames.encodeProteusMetadata('io.netifi.proteus.broker.info.BrokerManagementService', 'rejoin', Buffer.alloc(0), metadata || Buffer.alloc(0));
      const map = {};
      return this.rejoinTrace(map)(
        this._rs.requestResponse({
          data: dataBuf,
          metadata: metadataBuf
        }).map(function (payload) {
      ...
      BrokerManagementServiceClient.prototype.join = function join(message, metadata) {
        var dataBuf = Buffer.from(message.serializeBinary());
        var metadataBuf = proteus_js_frames.encodeProteusMetadata('io.netifi.proteus.broker.info.BrokerManagementService', 'join', Buffer.alloc(0), metadata || Buffer.alloc(0));
        const map = {};
        return this.joinTrace(map)(
          this._rs.requestResponse({
            data: dataBuf,
            metadata: metadataBuf
          }).map(function (payload) {
        ...
        BrokerManagementServiceClient.prototype.closeDestination = function closeDestination(messages, metadata) {
          var once = false;
          const map = {};
          return this.closeDestinationTrace(map)(
          ...
          BrokerManagementServiceClient.prototype.closeGroup = function closeGroup(messages, metadata) {
            var once = false;
            const map = {};
            return this.closeGroupTrace(map)(
              this._rs.requestChannel(messages.map(function (message) {
                var dataBuf = Buffer.from(message.serializeBinary());
                if (!once) {
                  once = true;
            ...
            BrokerManagementServiceClient.prototype.closeBroker = function closeBroker(messages, metadata) {
              var once = false;
              const map = {};
              return this.closeBrokerTrace(map)(
                this._rs.requestChannel(messages.map(function (message) {
                  var dataBuf = Buffer.from(message.serializeBinary());
                  if (!once) {
                    once = true;
              ...
              BrokerManagementServiceClient.prototype.closeDestinations = function closeDestinations(message, metadata) {
                var dataBuf = Buffer.from(message.serializeBinary());
                var metadataBuf = proteus_js_frames.encodeProteusMetadata('io.netifi.proteus.broker.info.BrokerManagementService', 'closeDestinations', Buffer.alloc(0), metadata || Buffer.alloc(0));
                const map = {};
                return this.closeDestinationsTrace(map)(
                  this._rs.requestResponse({
                    data: dataBuf,
                    metadata: metadataBuf
                  }).map(function (payload) {
                ...
                BrokerManagementServiceClient.prototype.closeBrokers = function closeBrokers(message, metadata) {
                  var dataBuf = Buffer.from(message.serializeBinary());
                  var metadataBuf = proteus_js_frames.encodeProteusMetadata('io.netifi.proteus.broker.info.BrokerManagementService', 'closeBrokers', Buffer.alloc(0), metadata || Buffer.alloc(0));
                  const map = {};
                  return this.closeBrokersTrace(map)(
                    this._rs.requestResponse({
                      data: dataBuf,
                      metadata: metadataBuf
                    }).map(function (payload) {
                  ...
                  BrokerManagementServiceClient.prototype.closeAll = function closeAll(message, metadata) {
                    var dataBuf = Buffer.from(message.serializeBinary());
                    var metadataBuf = proteus_js_frames.encodeProteusMetadata('io.netifi.proteus.broker.info.BrokerManagementService', 'closeAll', Buffer.alloc(0), metadata || Buffer.alloc(0));
                    const map = {};
                    return this.closeAllTrace(map)(
                      this._rs.requestResponse({
                        data: dataBuf,
                        metadata: metadataBuf
                      }).map(function (payload) {
